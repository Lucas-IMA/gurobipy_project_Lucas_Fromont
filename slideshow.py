...
if __name__ == "__main__":
    ...
    with gp.Model("slideshow") as model:
            ...
            Mat_photo_slide_position = model.addVars(nb_photos, nb_photos, 2, vtype=GRB.BINARY, name="Mat_photo_slide_position")

            # 1 si le tag t est dans la slide s, 0 sinon
            Mat_slide_tags = model.addVars(nb_photos, nb_tags, vtype=GRB.BINARY, name="Mat_slide_tags")

            # 1 si la slide s est utilisée, 0 sinon
            Utilisation_slide = model.addVars(nb_photos, vtype=GRB.BINARY, name="Utilisation_slide")

            # 1 si le tag t est commun entre les slides s et s+1, 0 sinon
            Tags_communs = model.addVars(nb_photos - 1, nb_tags, vtype=GRB.BINARY, name="Tags_communs")

            # 1 si le tag t est non commun entre les slides s et s+1, 0 sinon
            Tags_non_communs1 = model.addVars(nb_photos - 1, nb_tags, vtype=GRB.BINARY, name="Tags_non_communs1")

            # 1 si le tag t est non commun entre les slides s+1 et s, 0 sinon
            Tags_non_communs2 = model.addVars(nb_photos - 1, nb_tags, vtype=GRB.BINARY, name="Tags_non_communs2")

            # Nombre de tags communs entre deux slides consécutives
            Tags_communs_totaux = model.addVars(nb_photos - 1, vtype=GRB.INTEGER, name="Tags_communs_totaux")

            # Tags non communs présents dans la slide s mais pas dans la slide s+1
            Tags_non_communs1_totaux = model.addVars(nb_photos - 1, vtype=GRB.INTEGER, name="Tags_non_communs1_totaux")

            # Tags non communs présents dans la slide s+1 mais pas dans la slide s
            Tags_non_communs2_totaux = model.addVars(nb_photos - 1, vtype=GRB.INTEGER, name="Tags_non_communs2_totaux")
            ...
            for s in range(nb_photos):
                for p in range(2):
                    model.addConstr(Mat_photo_slide_position.sum('*', s, p) <= 1)
